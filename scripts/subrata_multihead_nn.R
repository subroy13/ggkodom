...
cat("\n----- Train: transition-level metrics (all transitions) -----\n")
compute_metrics(
...
#### PATIENT-LEVEL EVALUATION (competition metric) ####
# Competition scores positive-class F1 on RETURNED patients only.
# Transition-level F1 (above) includes ALL a1c_i -> a1c_j pairs.
# Patient-level F1 uses only the LAST transition per patient (-> a1c_2025).
# These are very different numbers — transition-level is inflated.
#
# WHY: Most transitions are easy same-state predictions (H stays H, L stays L
# between consecutive visits). The model spends capacity learning these trivial
# patterns. The hard question — will this patient's state CHANGE by 2025, across
# a long time gap — is diluted in training. This is a training-evaluation mismatch:
# loss treats all 36k transitions equally, competition only scores the ~9k final ones.
...
# Using state-specific thresholds from training transitions
cat("--- With state-specific thresholds (from train) ---\n")
val_patient_th <- ifelse(val_patient_level$start_state == 0, thresh0["best_th"], thresh1["best_th"])
compute_metrics(val_patient_level$prob > val_patient_th, val_patient_level$true_label)

# Best single threshold tuned on patient-level probs
cat("\n--- With best single threshold (tuned on val patients) ---\n")
patient_thresh <- best_f1_threshold(val_patient_level$prob, val_patient_level$true_label)
cat("Best threshold:", patient_thresh["best_th"], "\n")
cat("Best F1:", patient_thresh["best_f1"], "\n")
compute_metrics(val_patient_level$prob > patient_thresh["best_th"], val_patient_level$true_label)
