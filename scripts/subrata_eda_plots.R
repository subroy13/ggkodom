...
df$n_a1c <- rowSums(!is.na(df[, c(
  "a1c.1.estimated.result", "a1c.2.estimated.result",
...
med_cols <- c(
  "glp.1.orders.count", "insulin.orders.count", "metformin.orders.count",
...
print(data.frame(
  label = names(tbl), n = as.integer(tbl),
  pct = round(as.integer(tbl) / sum(tbl) * 100, 1)
))

cat("\n========== A1c-1 BY TARGET ==========\n")
print(aggregate(a1c.1.estimated.result ~ label,
  data = df,
  FUN = function(x) {
    round(c(
...
print(aggregate(cbind(raw_change, abs_change_12) ~ label,
  data = df2,
...
auto_cor <- aggregate(cbind(change_12, change_23) ~ label,
  data = df3,
  FUN = function(x) x
)
for (g in unique(df3$label)) {
  sub <- df3[df3$label == g, ]
  cat(sprintf(
    "  %s: cor(change_12, change_23) = %.3f, cor(r1, r2) = %.3f\n",
...
if (requireNamespace("infotheo", quietly = TRUE)) {
  ...
  print_cor_mi(df,
    cols = c(
      med_cols, "total_meds", "n_drug_classes",
      "ed.vist.count.count", "pcp.visit.count.count", "admission.count.count"
    ),
    labels = c(
      "GLP-1", "Insulin", "Metformin", "SGLT2", "Sulfonylurea", "DPP4",
  ...
  mi_cols_all <- c(
    "a1c.1.estimated.result", "insulin.orders.count", "ed.vist.count.count",
  ...
  print_cor_mi(df3,
    ...
    labels = c(
      "Mean", "Median", "a1c-3 (latest)", "a1c-1 (earliest)",
  ...
  print_cor_mi(df3,
    ...
    labels = c(
      "a1c-1 (earliest)", "a1c-3 (latest)", "equal (1/3)",
...
print(aggregate(cbind(adi_state, adi_national) ~ label,
  data = adi_sub,
  FUN = function(x) {
    round(c(
...
print(aggregate(target_num ~ high_a1c + on_insulin,
  data = df,
  FUN = function(x) c(rate = round(mean(x), 3), n = length(x))
))
...
df$age_group <- cut(df$age,
  breaks = c(20, 35, 45, 55, 65, 75, 85, 105),
...
if (requireNamespace("infotheo", quietly = TRUE)) {
  ...
  print_cor_mi(df,
    ...
    labels = c(
      "LDL timing", "HDL timing", "TC timing",
  ...
  mi_gender <- mutinformation(
    discretize(data.frame(x = df$gender_num[ok_g])),
    df$target_num[ok_g]
  )
  ...
  mi_race <- mutinformation(
    discretize(data.frame(x = df$race_num[ok_r])),
    df$target_num[ok_r]
  )
  ...
  mi_eth <- mutinformation(
    discretize(data.frame(x = df$eth_num[ok_e])),
    df$target_num[ok_e]
  )
  ...
  if (sum(ok_ins) > 0) {
    ...
    mi_ins <- mutinformation(
      discretize(data.frame(x = df$ins_num[ok_ins])),
...
p1 <- ggplot(df, aes(x = age, y = a1c_latest, color = label)) +
  ...
  labs(
    x = "Age (2025 - birth year)", y = "Latest A1c (%)",
    color = "2025 Status",
...
p2 <- ggplot(long, aes(x = day, y = a1c, group = id, color = label)) +
  ...
  scale_color_manual(values = c("Controlled" = "#1B9E77", "Uncontrolled" = "#D95F02")) +
  labs(
    x = "Days from first appointment", y = "A1c (%)",
    color = "2025 Status",
...
p3 <- ggplot(rate_df, aes(x = factor(n_a1c), y = rate)) +
  ...
  labs(
    x = "Number of A1c Readings", y = "Uncontrolled Rate (%)",
...
p3b <- ggplot(n_a1c_pct, aes(x = n_a1c, y = pct, fill = label)) +
  ...
  labs(
    x = "Number of A1c Readings", y = "Percent within group",
    fill = "2025 Status",
...
p4 <- ggplot(df_with_date, aes(x = a1c.2025.collection.date.time.days.from.reference, fill = label)) +
  ...
  scale_fill_manual(values = c("Controlled" = "#1B9E77", "Uncontrolled" = "#D95F02")) +
  labs(
    x = "Days from first appointment to 2025 evaluation",
...
p5b <- ggplot(med_means, aes(x = reorder(drug, orders), y = orders, fill = label)) +
  ...
  coord_flip() +
  labs(
    x = "", y = "Mean Order Count", fill = "2025 Status",
...
p6 <- ggplot(df, aes(x = a1c.1.estimated.result, fill = label)) +
  ...
  scale_fill_manual(values = c("Controlled" = "#1B9E77", "Uncontrolled" = "#D95F02")) +
  labs(
    x = "A1c-1 (earliest reading, %)", y = "Count", fill = "2025 Status",
...
box_vars <- data.frame(
  col = c(
    "a1c.1.estimated.result", "date.of.birth", "ed.vist.count.count",
...
p10 <- ggplot(df2_plot, aes(x = a1c.2.estimated.result, y = a1c.1.estimated.result)) +
  ...
  labs(
    x = "A1c-2 (later reading)", y = "A1c-1 (earliest reading)",
...
p11 <- ggplot(df2_plot, aes(x = abs_change, fill = label)) +
  ...
  labs(
    x = "|A1c-1 - A1c-2|", y = "Count", fill = "2025 Status",
...
p12 <- ggplot(long3, aes(x = reading, y = a1c, color = label)) +
  ...
  labs(
    x = "Reading", y = "A1c (%)", color = "2025 Status",
...
p15 <- ggplot(adi_plot, aes(x = label, y = adi_state, fill = label)) +
  ...
  scale_fill_manual(values = c("Controlled" = "#1B9E77", "Uncontrolled" = "#D95F02")) +
  labs(
    x = "", y = "ADI State Rank (higher = more deprived)",
...
miss_cols <- c(
  "weight.estimated.result", "height.estimated.result",
  "weight.measurement.date.time.days.from.reference",
  "height.measurement.date.time.days.from.reference",
  "ldl.estimated.result", "hdl.estimated.result",
  "total.cholesterol.estimated.result",
...
miss_labels <- c(
  "Weight", "Height", "Weight timing", "Height timing",
...
profile <- aggregate(
  cbind(
    a1c.1.estimated.result, insulin.orders.count,
    ed.vist.count.count, n_a1c, total_meds, a1c_latest
  ) ~ returned_2025,
  data = df, FUN = mean, na.rm = TRUE
)
names(profile) <- c(
  "returned", "mean_a1c1", "mean_insulin", "mean_ed",
